...
const fetch = async (url) => await new Promise((resolve, reject) => request({ url, method: 'GET' }, (err, resp) => err ? reject(err) : resolve(JSON.parse(resp.body))));
...
exports.fetchBitcoinPriceHistory = async (confirmationTime) => {
  ...
  const cgResponse = await fetch('https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days=.1&interval=minute');
...
exports.getDefaultBitcoinClient = () => new exports.BitcoinClient({
  network: 'mainnet',
