...
const setValue = async (level, key: string, value: string) =>
  await new Promise<void>((resolve, reject) =>
    level.put(key, value, (err) => (err ? reject(err) : resolve()))
  );
const delValue = async (level, key: string) =>
  await new Promise<void>((resolve, reject) =>
    level.del(key, (err) => (err ? reject(err) : resolve()))
  );
...
const requestToKey = (request) =>
  ethers.utils.solidityKeccak256(["bytes"], [request.signature]);
...
export class LevelDBPersistenceAdapter
  implements PersistenceAdapter<LocalStorageBackendType, LocalStorageKeyType>
{
  ...
  async get(key) {
    return (
      JSON.parse((await getValue(this.backend, toKey(key))) || "0") || null
    );
  }
  async getIndex(key) {
  ...
  async set(transferRequest) {
    ...
    await setValue(
      this.backend,
      toKey(key),
      JSON.stringify(requestToPlain(transferRequest))
    );
    return key;
  }
  async remove(key) {
