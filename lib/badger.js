...
const applyFee = (exports.applyFee = async (amountIn, fee, multiplier) => {
  ...
  const gasFee = await computeRenBTCGasFee(GAS_COST.add(keeperReward.div(gasPrice)), gasPrice);
  const mintFee = applyRatio(amountIn, fee);
  const totalFees = gasFee.add(mintFee);

  return {gasFee: gasFee, mintFee: mintFee, totalFees: totalFees};
});
...
const fromUSDC = async (amount) => {
	...
	let trade = null;
	try {
		trade = new UNISWAP.Trade(
			route,
			new UNISWAP.TokenAmount(USDC, ethers.BigNumber.from(amount).toString()),
...
const toUSDC = (exports.toUSDC = async (amount) => {
	try {
		...
		const trade = new UNISWAP.Trade(
			route,
			new UNISWAP.TokenAmount(WBTC, ethers.BigNumber.from(wbtcOut).toString()),
			UNISWAP.TradeType.EXACT_INPUT,
		);
...
const computeTransferOutput = (exports.computeTransferOutput = async ({ module, amount }) => {
	switch (module) {
...
const WBTCFromETH = async (amount) => {
	...
	const trade = new UNISWAP.Trade(
		route,
		new UNISWAP.TokenAmount(WETH, ethers.BigNumber.from(amount).toString()),
		UNISWAP.TradeType.EXACT_INPUT,
	);
	const result = await renBTCFromWBTC(ethers.BigNumber.from(trade.outputAmount.raw.toString(10)));
	return result;
...
const getConvertedAmount = (exports.getConvertedAmount = async (asset, amount) => {
	switch (asset) {
		case fixtures.ETHEREUM.WBTC:
...
const computeOutputBTC = (exports.computeOutputBTC = async (burnRequest) => {
  ...
  const convertedAmount = await getConvertedAmount(asset, amount);
	...
	switch (asset) {
		case fixtures.ETHEREUM.WBTC:
			return applyRenVMFee(await deductBurnFee(convertedAmount, 1));
		case fixtures.ETHEREUM.renBTC:
			return applyRenVMFee(await deductBurnFee(convertedAmount, 1));
		case fixtures.ETHEREUM.USDC:
			return applyRenVMFee(await deductBurnFee(convertedAmount, 1));
		case ethers.constants.AddressZero:
			return applyRenVMFee(await deductBurnFee(convertedAmount));
		default:
