...
const getRenBTCForOneETHPrice = async () => {
  ...
  const pair = await UNISWAP.Fetcher.fetchPairData(renBTC, UNISWAP.WETH[renBTC.chainId]);
...
const fromUSDC = async (amount) => {
  ...
  const route = new UNISWAP.Route([await UNISWAP.Fetcher.fetchPairData(renBTC, WETH), await UNISWAP.Fetcher.fetchPairData(WETH, USDC)], renBTC); 
  ...
  return ethers.BigNumber.from(trade.outputAmount.toFixed(0));
};
