...
const applyFee = async (amountIn, fee, gasPrice) => {
...
const transferFee = ethers.utils.parseEther('0.003');

const deductFee = async (amount, fee) => {
  amount = ethers.BigNumber.from(amount);
  const gasPrice = await provider.getGasPrice();
  return amount.sub(await applyFee(amount, fee, gasPrice));
};
...
const computeOutputBTCIgnore = exports.computeOutputBTCIgnore = async (burnRequest) => {
...
const computeOutputBTC = exports.computeOutputBTC = async (mintRequest) => {
  const { asset, amount, type } = mintRequest;
  let feeAmount = 0;
  switch(type) {
    case 'transfer':
  ...
  switch (asset) {
    case 'wBTC' || fixtures.ETHEREUM.WBTC:
      return applyRenVMFee(await deductFee(await renCrv.get_dy(1, 0, amount), feeAmount));
    case 'renBTC' || fixtures.ETHEREUM.renBTC:
      return applyRenVMFee(await deductFee(amount, feeAmount));
    case 'USDC' || fixtures.ETHEREUM.USDC:
      return applyRenVMFee(await deductFee(await fromUSDC(amount), feeAmount)); 
    default:
      console.log('no asset found for computeOutputBTC:' + asset);
      return amount;
  }
