...
class ZeroUser extends EventEmitter {
  ...
  async publishRequest(
    ...
    requestType: string = "transfer"
  ) {
    const requestFromTemplate = requestTemplate
      ? Object.fromEntries(
          Object.entries(request).filter(([k, v]) =>
            requestTemplate.includes(k)
          )
        )
      : request;
    ...
    if (this.keepers.length === 0) {
      this.log.error(
        `Cannot publish ${requestType} request if no keepers are found`
      );
      return result;
    }
    try {
      ...
      for (const keeper of this.keepers) {
        ...
        if (ackReceived !== true) {
          try {
            ...
            const { stream } = await this.conn.dialProtocol(
              peer,
              "/zero/1.1.0/dispatch"
            );
            pipe(JSON.stringify(requestFromTemplate), lp.encode(), stream.sink);
            this.log.info(
              `Published ${requestType} request to ${keeper}. Waiting for keeper confirmation.`
            );
          } catch (e: any) {
...
class ZeroKeeper {
  ...
  constructor(
    ...
    this.conn = connection;
    this.conn.on("peer:discovery", () =>
      console.log("discovered from keeper!")
    );
    this.dispatches = [];
  ...
  async setTxDispatcher(callback: Function) {
    const handler = async (duplex: any) => {
      ...
      pipe(stream.source, lp.decode(), async (rawData: any) => {
        ...
        callback(
          transferRequest,
          this.makeReplyDispatcher(duplex.connection.remotePeer),
          errors
        );
      });
    };
