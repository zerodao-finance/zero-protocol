...
class ZeroUser {
  ...
  async subscribeKeepers() {
    this.conn.pubsub.on('zero.keepers', async (message) => {
      const {msg, peerInfo: {peerId}} = fromBufferToJSON(message)
      if(!this.keepers.includes(peerId)) {
        try {
          ...
          this.log.info(`Found keeper: ${peerId}`)
        } catch (e) {
  ...
  async publishTransferRequest(transferRequest) {
    ...
    try {
      ...
      await this.conn.handle('/zero/user/confirmation', async (msg) => {
        const {txConfirmation} = fromBufferToJSON(msg)
        ackReceived = true;
      ...
      for(const keeper of this.keepers) {
        if(ackReceived !== true) {
          try {
            await this.conn.dialProtocol(keeper, '/zero/keeper/dispatch', fromJSONtoBuffer(transferRequest))
          } catch (e) {
...
class ZeroKeeper {
    ...
    setInterval(async () => {
      try {
        await this.conn.pubsub.publish('zero.keepers', fromJSONtoBuffer(
          {
            msg: address,
            peerInfo: {peerId: this.conn.peerId.toB58String()}
          }
        ))
      } catch (e) {
        console.debug(e)
  ...
  async setTxDispatcher(callback) {
    await this.conn.handle('/zero/keeper/dispatch', callback)
  }
}
