import { DelegateUnderwriterTransferRequest, createZeroKeeper, TransferRequest } from './zero';
...
export const createMockKeeper = async (provider) => {
	...
	keeper.setTxDispatcher(async (transferRequest) => {
		const delegate = new DelegateUnderwriterTransferRequest(transferRequest);
		try {
			const loan_result = await delegate.dry(keeperSigner, { from: await keeperSigner.getAddress() });
			console.log('Loan Result', loan_result);
		} catch (err) {
			console.log('ERROR', err);
		}
		const mint = await delegate.submitToRenVM(true);
		...
		delegate.waitForSignature = async () => {
			await new Promise((resolve) => setTimeout(resolve, 1000));
			return {
				amount: ethers.BigNumber.from(delegate.amount).sub(ethers.utils.parseUnits('0.0015', 8)).toString(),
