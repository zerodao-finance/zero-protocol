...
const ShifterBorrowProxy = {};
const ShifterBorrowProxyTest = {};
const shifterBorrowProxyBytecode = Boolean(process.env.DEBUG || process.env.REACT_APP_CHAIN === 'test')
	? ShifterBorrowProxyTest.bytecode
	: ShifterBorrowProxy.bytecode;
...
const maybeCoerceToPHash = (params) =>
	Array.isArray(params)
		? !params[1] || params[1].length === 0
			? NULL_PHASH
...
const Exports = {};
const InitializationActionsABI = [];

const encodeInitializationActions = (input) =>
	abi.encode(
...
const computeLiquidityRequestHash = ({
	...
	solidityKeccak256(
		...
		[shifterPool, token, nonce, amount, gasRequested, forbidLoan, encodeInitializationActions(actions)],
	);

const assembleCloneCode = require('./assemble-clone-code');

const computeBorrowProxyAddress = ({ shifterPool, borrower, token, nonce, amount, forbidLoan, actions }) => {
	const salt = solidityKeccak256(
		['address', 'address', 'bytes32', 'uint256', 'bool', 'bytes'],
		[borrower, token, nonce, amount, forbidLoan, encodeInitializationActions(actions)],
	);
	const implementation = getCreate2Address(
		shifterPool,
		solidityKeccak256(['string'], ['borrow-proxy-implementation']),
		solidityKeccak256(['bytes'], [shifterBorrowProxyBytecode]),
	);
...
const computeShiftInTxHash = ({ renContract, utxo, g }) =>
	toBase64(
		solidityKeccak256(
			['string'],
...
const computeP = (nonce, data) => abi.encode(['uint256', 'bytes'], [nonce, data]);

const maybeCoerceToNHash = (input) => (typeof input === 'object' ? computeNHash(input) : input);

const computeHashForDarknodeSignature = ({ p, n, amount, to, tokenAddress }) =>
	keccakAbiEncoded(
...
const computeGatewayAddress = ({ isTestnet, g, mpkh }) =>
	new Script()
