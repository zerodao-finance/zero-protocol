...
const toAddress = (contractOrAddress) =>
...
const setConverter = async (controller, source, target, converter) => {
  const [sourceAddress, targetAddress] = [source, target].map(
    (v) => deployParameters[network][v] || v
  );
...
module.exports = async ({
  ...
  const zeroUnderwriterLockBytecodeLib = await deployFixedAddress(
  ...
  const zeroControllerFactory = await hre.ethers.getContractFactory(
    ...
    process.env.CHAIN !== 'MATIC'
      ...
      : {
          libraries: {
            ZeroUnderwriterLockBytecodeLib:
              zeroUnderwriterLockBytecodeLib.address,
          },
        }
  );
  const zeroController =
    process.env.CHAIN !== 'MATIC'
      ? await deployProxyFixedAddress(zeroControllerFactory, [
          deployer,
          deployer,
        ])
  ...
  if (isLocalhost) {
    const meta = await deployFixedAddress(
      process.env.CHAIN !== 'MATIC' ? 'MetaExecutorEthereum' : 'MetaExecutor',
      {
        args: [controller.address, ethers.utils.parseUnits('15', 8), '100000'],
        contractName:
          process.env.CHAIN !== 'MATIC'
            ? 'MetaExecutorEthereum'
            : 'MetaExecutor',
        libraries: {},
        from: deployer,
      }
  ...
  await controller.approveStrategy(
    deployParameters[network]['renBTC'],
  ...
  */
  ...
  switch (network) {
    case 'ETHEREUM':
      ...
      // Curve renBTC -> wBTC
      var renBTCToWBTCTx = await curveFactory.createWrapper(
        false,
        0,
        1,
        deployParameters[network]['Curve_SBTC']
      ...
      break;
    case 'MATIC':
      const sushiFactory = await ethers.getContract(
        'ZeroUniswapFactory',
        deployer
      );
      console.log('MATIC');
      // Curve wBTC -> renBTC
      var wBTCToRenBTCTx = await curveFactory.createWrapper(
        true,
        0,
        1,
      ...
      await setConverter(
        ...
        'wNative',
        'WBTC',
        '0x7157d98368923a298C0882a503cF44353A847F37'
      );
      // Sushi wBTC -> wNative
  ...
  await controller.setGasParameters(
