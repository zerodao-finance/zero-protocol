...
//@ts-ignore
hre.ethers.providers.BaseProvider.prototype.getGasPrice = async () =>
...
const toAddress = (contractOrAddress: any): string =>
  contractOrAddress.address || contractOrAddress;
const mintRenBTC = async (amount: any, signer?: any) => {
  ...
  //@ts-ignore
  const btcGateway = new ethers.Contract(
...
const convert = async (
  ...
  const [tokenInAddress, tokenOutAddress] = [tokenIn, tokenOut].map((v) =>
    toAddress(v)
  );
  const swapAddress = await controller.converters(
    tokenInAddress,
    tokenOutAddress
  );
...
const deployUnderwriter = async () => {
  ...
  /*
	...
	underwriterAddress = (await underwriterFactory.deploy(controller.address)).address;
       */
  underwriterAddress = (await ethers.getContract("DelegateUnderwriter"))
    .address;
...
const mintUnderwriterNFTIfNotMinted = async () => {
  ...
  const lock = await controller.provider.getCode(
    await controller.lockFor(underwriterAddress)
  );
  if (lock === "0x")
...
const getStrategyContract = async (signer) => {
  if (process.env.CHAIN === "ETHEREUM")
    return await getContract("StrategyRenVMEthereum", signer);
  if (process.env.CHAIN === "ARBITRUM")
    return await getContract("StrategyRenVMArbitrum", signer);
  ...
  return {
    ...
    swapModule:
      network === "ARBITRUM"
        ...
        : await getContract("BadgerBridge", signer),
    convertModule:
      network === "ARBITRUM"
        ? await getContract("ArbitrumConvert", signer)
        : network === "MATIC"
              ...
              await Promise.all(
                Object.keys(tokens).map(async (token) => {
                  if (token === "ETH") {
                    ...
                    return [
                      token,
                      String(Number(utils.formatEther(balance)).toFixed(2)),
                    ];
                  } else {
...
const getUnderwriter = async () => {
  ...
  const underwriterFactory = await getContractFactory(
    "DelegateUnderwriter",
    signer
  );
  return {
    underwriterFactory,
    underwriterAddress,
    underwriter: new Contract(
      underwriterAddress,
      underwriterFactory.interface,
      signer
    ),
    underwriterImpl: new Contract(
      underwriterAddress,
...
describe("Zero", () => {
  ...
  before(async () => {
    ...
    await btcVault.transfer(
      lock,
      btcVault.balanceOf(btcVault.signer.getAddress())
    );
  });
  ...
  it("mock gateway should work", async () => {
    ...
    expect(
      Number(ethers.utils.formatUnits(await renbtc.balanceOf(signerAddress), 8))
    ).to.be.gt(0);
  });

  it.only("Swap ETH -> wETH -> ETH", async () => {
  ...
  it("Swap renBTC -> WBTC -> renBTC", async () => {
    ...
    expect(
      Number(await renBTC.balanceOf(await signer.getAddress())) > 0,
      "The swap amounts dont add up"
    );
  });
  ...
  it("should deposit funds then withdraw funds back from vault", async () => {
    ...
    const beforeBalance =
      (await renBTC.balanceOf(btcVault.address)).toNumber() /
      (await renBTC.decimals());
    ...
    expect(
      beforeBalance + Number(addedAmount) == afterBalance,
      "Balances not adding up"
    );
  });
  ...
  it("should take out, make a swap with, then repay a small loan", async () => {
    const { signer, controller, swapModule, convertModule, btcVault } =
      await getFixtures();
    const { underwriter, underwriterImpl } = await getUnderwriter();

    const renbtc = new ethers.Contract(
      await btcVault.token(),
      btcVault.interface,
      signer
    );
    await renbtc.approve(btcVault.address, ethers.constants.MaxUint256);
    ...
    const transferRequest = new UnderwriterTransferRequest({
      ...
      nonce:
        "0xb67ed6c41ea6f5b7395f005ceb172eb093273396d1e5bb49d919c4df396e0d5a",
      pNonce:
        "0x0153c5fa086b7eceef6ec52b6b96381ee6f16852a6ace5b742f239296b4cd901",
      chainId:
        process.env.CHAIN === "ARBITRUM"
          ? 42161
          : process.env.CHAIN === "ETHEREUM"
