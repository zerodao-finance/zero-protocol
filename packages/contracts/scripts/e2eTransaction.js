...
const { UnderwriterTransferRequest, TransferRequest } = require('../lib/zero');
const { ZeroController, Swap } = require('../dist/lib/util/deployed-contracts');
const { Contract, Wallet, providers, utils } = require('ethers');
const { address: underwriterAddress, abi: underwriterAbi } = require('../deployments/matic/DelegateUnderwriter.json');
...
const keeperCallback = async (msg) => {
	...
	try {
		const tr = new UnderwriterTransferRequest(msg);
...
const main = async () => {
	...
	const Underwriter = new Contract(underwriterAddress, underwriterAbi, signer);
	...
	const lock = await Controller.provider.getCode(
		await Controller.lockFor(Underwriter.address, {
