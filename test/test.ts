...
import { Wallet, Contract, constants, utils, ContractFactory } from 'ethers';
...
const getContractFactory: (...args: any[]) => Promise<ContractFactory> = async (...args) => {
    try {
...
let underwriterAddress = constants.AddressZero;

const deployUnderwriter = async () => {
    console.log('deploying underwriter')
...
const generateTransferRequest = async (amount: number, module?: string, data?: string) => {
    ...
    return new TransferRequest({
        module: module ? module : process.env.CHAIN === 'ARBITRUM' ? convertModule.address : swapModule.address,
        to: signerAddress,
...
*/
    ...
    it('should test the swap contract (_swapv2test)', async () => {
        ...
        const { signer, controller, btcVault } = await getFixtures();
        const swapV2Module = await getContract('SwapV2')
        const { underwriter, underwriterImpl } = await getUnderwriter();
        const renbtc = new ethers.Contract(await btcVault.token(), btcVault.interface, signer);
        await renbtc.approve(btcVault.address, ethers.constants.MaxUint256);
