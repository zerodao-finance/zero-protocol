...
describe("BadgerBridgeZeroController", () => {
  before(async () => {
    ...
    await new hre.ethers.Contract(
      ...
      (
        await hre.ethers.getContract("BadgerBridgeZeroController")
      ).address
    );
    await hre.network.provider.send("hardhat_impersonateAccount", [
    ...
    (await hre.ethers.getContract("BadgerBridgeZeroController")).approveUpgrade(
      true
    );
  });
  it("should do a transfer", async () => {
    ...
    const transferRequest = new UnderwriterTransferRequest({
      ...
      chainId,
      data: utils.defaultAbiCoder.encode(["uint256", "uint256"], ["1", "0"]),
