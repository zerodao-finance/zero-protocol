...
const mintRenBTC = async (amount, signer) => {
    const abi = [
...
const getContractFactory = async (...args) => {
    try {
...
const convert = async (controller, tokenIn, tokenOut, amount, signer) => {
...
const underwriterDeposit = async (amountOfRenBTC) => {
...
const getBalances = async () => {
    ...
    console.table(Object.fromEntries(await Promise.all(Object.keys(wallets).map(async (name) => {
...
describe('Zero', () => {
    ...
    it('should take out, make a swap with, then repay a small loan', async () => {
        ...
        await underwriter.loan(transferRequest.to, transferRequest.asset, transferRequest.amount, transferRequest.pNonce, transferRequest.module, transferRequest.data, signature);
        await getBalances();
        console.log('\nRepaying loan...');
    ...
    it('should take out, make a swap with, then repay a large loan', async () => {
        ...
        await underwriterImpl.loan(transferRequest.to, transferRequest.asset, transferRequest.amount, transferRequest.pNonce, transferRequest.module, transferRequest.data, signature);
        await getBalances();
        console.log('\nRepaying loan...');
        await underwriterImpl.repay(underwriter.address, //underwriter
    ...
    it('should call fallback mint and return funds', async () => {
        ...
        await controller.fallbackMint(underwriter.address, //underwriter,
        transferRequest.to, //to
        transferRequest.asset, //asset
        transferRequest.amount, //amount
        String(Number(transferRequest.amount) - 1000), //actualAmount
