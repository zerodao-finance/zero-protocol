		...
		try {
			...
			await this.conn.handle('/zero/user/confirmation', async ({ stream }) => {
				pipe(stream.source, lp.decode(), async (rawData) => {
					...
					try {
						for (
							var rawData_1 = __asyncValues(rawData), rawData_1_1;
							(rawData_1_1 = await rawData_1.next()), !rawData_1_1.done;
	...
	async setTxDispatcher(callback) {
		const handler = async (duplex) => {
			...
			pipe(stream.source, lp.decode(), async (rawData) => {
				...
				try {
					for (
						var rawData_2 = __asyncValues(rawData), rawData_2_1;
						(rawData_2_1 = await rawData_2.next()), !rawData_2_1.done;
