...
pragma solidity >=0.6.0<0.8.0<0.8.0;
...
contract OlympusBondDepository is Ownable {
	...
	address public immutable ZERO; // token given as payment for bond
	address public immutable principle; // token used to create bond
	address public immutable treasury; // mints ZERO when receives principle
	...
	struct Bond {
		uint256 payout; // ZERO remaining to be paid
	...
	function deposit(
		...
		require(payout >= 10000000, 'Bond too small'); // must be > 0.01 ZERO ( underflow protection )
		...
		/**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) ZERO
         */
		...
		if (fee != 0) {
			...
			IERC20(ZERO).safeTransfer(DAO, fee);
		}
	...
	function stakeOrSend(
		...
		if (!_stake) {
			...
			IERC20(ZERO).transfer(_recipient, _amount); // send payout
		} else {
			// if user wants to stake
			if (useHelper) {
				// use if staking warmup is 0
				IERC20(ZERO).approve(stakingHelper, _amount);
				IStakingHelper(stakingHelper).stake(_amount, _recipient);
			} else {
				IERC20(ZERO).approve(staking, _amount);
	...
	function maxPayout() public view returns (uint256) {
		return IERC20(ZERO).totalSupply().mul(terms.maxPayout).div(100000);
	}
	...
	/**
	 *  @notice calculate current ratio of debt to ZERO supply
	 *  @return debtRatio_ uint
	 */
	function debtRatio() public view returns (uint256 debtRatio_) {
		uint256 supply = IERC20(ZERO).totalSupply();
	...
	/**
	 *  @notice calculate amount of ZERO available for claim by depositor
	 *  @param _depositor address
	...
	/**
	 *  @notice allow anyone to send lost tokens (excluding principle or ZERO) to the DAO
	 *  @return bool
	 */
