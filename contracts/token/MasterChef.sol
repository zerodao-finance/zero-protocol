...
// MasterChef is the master of Zero. He can make Zero and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    function pendingZero(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 zeroReward =
                multiplier.mul(zeroPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accZeroPerShare = accZeroPerShare.add(
                zeroReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accZeroPerShare = pool.accZeroPerShare.add(
            zeroReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accZeroPerShare).div(1e12).sub(
                    user.rewardDebt
                );
