...
contract OlympusTreasury is Ownable {
	...
	uint256 public sZEROQueue; // Delays change to sZERO address
	...
	function incurDebt(uint256 _amount, address _token) external {
		...
		uint256 maximumDebt = IERC20(sZERO).balanceOf(msg.sender); // Can only borrow against sZERO held
	...
	/**
        ...
        @param _amount uint
     */
	function repayDebtWithZERO(uint256 _amount) external {
		require(isDebtor[msg.sender], 'Not approved');
	...
	function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
		...
		} else if (_managing == MANAGING.SZERO) {
			// 9
