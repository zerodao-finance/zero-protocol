...
contract OlympusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SZERO }
    ...
    address public sZERO;
    uint public sZEROQueue; // Delays change to sZERO address
    ...
    constructor (
       ...
       isLiquidityToken[ _ZERODAI ] = true;
       liquidityTokens.push( _ZERODAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for ZERO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ZERO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ZERO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ZERO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IZEROERC20( ZERO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sZERO ).balanceOf( msg.sender ); // Can only borrow against sZERO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ZERO
        @param _amount uint
     */
    function repayDebtWithZERO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IZEROERC20( ZERO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ZERO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ZERO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ZERO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ZERO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ZERO decimals
            value_ = _amount.mul( 10 ** IERC20( ZERO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SZERO ) { // 9
            sZEROQueue = block.number.add( blocksNeededForQueue );
        } else return false;
