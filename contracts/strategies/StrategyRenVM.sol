...
contract StrategyRenVM {
    ...
    address[] public constant renBTCPath = [renBTC, wETH, USDC];
    address[] public constant wETHPath = [wETH, USDC];
    address[] public constant wETHtoRenBTCPath = [wETH, renBTC];
    ...
    function deposit() external virtual {
        ...
        if (_want > 0) { //TODO should be min threshold, shouldn't need to be 0
    ...
    function withdrawAll() external virtual onlyController {
        ...
        IUniswapV2Router02(router).swapExactETHForTokens(_eth, [weth, want], address(controller), block.timestamp);
        IERC20(want).transfer(address(controller), IERC20(want).balanceOf(address(this)));
