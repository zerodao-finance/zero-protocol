...
contract ArbitrumMIMConvert {
  ...
  function isActive(ArbitrumConvertLib.ConvertRecord storage record) internal view returns (bool) {
    ...
    (uint256 ratio) = abi.decode(_data, (uint256));
		...
		outstanding[_nonce] = ArbitrumConvertLib.ConvertRecord({qty: amountSwappedMIM, when: uint64(block.timestamp), qtyETH: amountSwappedETH });
  }

	function swapTokens(
		uint256 _amountIn,
    uint256 _ratio
	) internal returns (uint256 amountSwappedETH, uint256 amountSwappedMIM) {
  ...
  function swapTokensBack(ArbitrumConvertLib.ConvertRecord storage record) internal returns (uint256 amountReturned) {
    ...
    uint256 amountSwappedFromETH = ICurveETHUInt256(tricryptoArbitrum).exchange{ value: record.qtyETH }(2, 1, record.qtyETH, 0, true);
    uint256 amountSwappedFromUsdt = ICurveETHUInt256(tricryptoArbitrum).exchange(0, 1, usdtOut, 0, false);
    amountReturned = IRenCrvArbitrum(renCrvArbitrum).exchange(1, 0, amountSwappedFromETH.add(amountSwappedFromUsdt), 0, address(this));
  } 
	function repayLoan(
		address _to,
		address _asset,
