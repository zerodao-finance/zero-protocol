...
contract ZeroController is ControllerUpgradeable, OwnableUpgradeable, ERC721Upgradeable {
   ...
   bytes32 private constant ZERO_DOMAIN_NAME_HASH = keccak256("ZeroUnderwriterController.RenVMBorrowMessage");
   bytes32 private constant ZERO_DOMAIN_VERSION_HASH = keccak256("v2");
   bytes32 private constant ZERO_RENVM_BORROW_MESSAGE_TYPE_HASH = keccak256("RenVMBorrowMessage(address module,uint256 amount,address underwriter,uint256 pNonce,bytes pData)");
   ...
   modifier onlyUnderwriter {
     require(ownerOf(lockFor(msg.sender)) != address(0x0), "must be called by underwriter");o
     _;
   }
   ...
   function repay(address underwriter, address to, address asset, uint256 amount. uint256 actualAmount, uint256 nonce, address module, bytes32 nHash, bytes memory data, bytes memory signature) public {
     ...
     uint256 amount = IGateway(getGateway(asset)).mint(keccak256(abi.encode(nonce, data)), actualAmount, nHash, signature);
     depositAll(asset);
   }
   function toTypedDataHash(LoanParams memory params, address underwriter) internal pure returns (bytes32 result) {
     result = ECDSA.toTypedDataHash(ZERO_DOMAIN_SEPARATOR, keccak256(abi.encode(ZERO_RENVM_BORROW_MESSAGE_TYPE_HASH, params.asset, params.amount, underwtiter, params.nonce, params.module, params.data)));
   }
   function loan(address to, address asset, uint256 amount, uint256 nonce, address module, bytes memory data, bytes memory userSignature) public onlyUnderwriter {
     ZeroLib.LoanParams memory params = ZeroLib.LoanParams({
       to: to,
       asset: asset,
       amount: amount,
     ...
     require(ECDSA.recover(toTypedDataHash(params, msg.sender), userSignature) == params.to, "invalid signature");   
