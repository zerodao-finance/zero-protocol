...
contract ERC20WithRate is Initializable, Ownable, ERC20 {
  ...
  function initialize(address _nextOwner, uint256 _initialRate) public initializer {
    Ownable.initialize(_nextOwner);
  ...
  function fromUnderlying(uint256 _amountUnderlying) public view returns (uint256) {
...
contract ERC20WithPermit is Initializable, ERC20, ERC20Detailed {
  ...
  function permit(
    ...
    require(expiry == 0 || now <= expiry, "ERC20WithRate: permit has expired");
  ...
  CanReclaimTokens
{
  function initialize(
    ...
    ERC20WithPermit.initialize(_chainId, _version, _name, _symbol, _decimals);
    Claimable.initialize(_nextOwner);
...
contract GatewayLogicV1 is Initializable, Claimable, CanReclaimTokens, IGateway, GatewayStateV1 {
  using SafeMath for uint256;

  event LogMintAuthorityUpdated(address indexed _newMintAuthority);
  event LogMint(address indexed _to, uint256 _amount, uint256 indexed _n, bytes32 indexed _signedMessageHash);
  event LogBurn(bytes _to, uint256 _amount, uint256 indexed _n, bytes indexed _indexedTo);

  modifier onlyOwnerOrMintAuthority() {
    require(msg.sender == mintAuthority || msg.sender == owner(), "Gateway: caller is not the owner or mint authority");
    _;
  }
  ...
  function transferTokenOwnership(GatewayLogicV1 _nextTokenOwner) public onlyOwner {
  ...
  function updateMintAuthority(address _nextMintAuthority) public onlyOwnerOrMintAuthority {
    require(_nextMintAuthority != address(0), "Gateway: mintAuthority cannot be set to address zero");
  ...
  function mint(
    ...
    bytes32 signedMessageHash = hashForSignature(_pHash, _amountUnderlying, msg.sender, _nHash);
    ...
    uint256 absoluteFeeScaled = amountScaled.mul(mintFee).div(BIPS_DENOMINATOR);
    uint256 receivedAmountScaled = amountScaled.sub(absoluteFeeScaled, "Gateway: fee exceeds amount");
    ...
    emit LogMint(msg.sender, receivedAmountUnderlying, nextN, signedMessageHash);
    nextN += 1;
  ...
  function burn(bytes memory _to, uint256 _amount) public returns (uint256) {
    ...
    uint256 amountAfterFee = _amount.sub(fee, "Gateway: fee exceeds amount");
    ...
    require(amountAfterFeeUnderlying > minimumBurnAmount, "Gateway: amount is less than the minimum burn amount");
